<script>
  ...
  // image props
  export let img = "";
  export let back = "./img/card-back.webp";
  export let foil = "";
  export let mask = "";
  ...
  let back_img = back;
  let front_img = "";
  // remote urls and local paths (starting with "/" or "./") are used as-is,
  // anything else is treated as a card id on the pokemontcg CDN
  let img_base = (img.startsWith("http") || img.startsWith("/") || img.startsWith("./")) ? "" : "https://images.pokemontcg.io/";
  ...
  // 悬停卡片时通知 App（Patterns 旁的 ShuffleCode 洗牌）
  const cardEnter = () => {
    window.dispatchEvent(new CustomEvent("card-hover", { detail: id }));
  };

  const cardLeave = () => {
    window.dispatchEvent(new CustomEvent("card-leave"));
  };
